<template>
  <div 
    ...
    @pointerdown.stop
    @wheel.stop
    class="fixed inset-0 z-50 bg-black/75 backdrop-blur-md flex items-center justify-center p-2.5 sm:p-4 select-none animate-in fade-in duration-200 pt-safe pb-safe"
  >
    <div 
      class="glass-panel border border-amber-500/50 w-full max-w-2xl rounded-3xl p-4 sm:p-5 shadow-2xl bg-slate-900/98 flex flex-col gap-3.5 sm:gap-4 text-xs text-slate-200 max-h-[88dvh] overflow-hidden animate-in zoom-in-95 duration-200"
    >
      <!-- Modal Header -->
