<template>
  <div
    ...
    <UiCard v-if="towerStore.selectedPlacedTower" variant="slate" padding="sm"
      custom-class="absolute! right-4 -top-[calc(100%+2rem)] border-sky-500/60 shadow-2xl backdrop-blur-xl bg-slate-950/95 pointer-events-auto flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 sm:gap-4 text-xs text-slate-200 w-full max-w-lg animate-in slide-in-from-bottom-2 duration-150"
      ...
      <div class="flex items-center gap-3 min-w-0">
        ...
        <div class="flex flex-col min-w-0 text-left">
          <div class="flex items-center gap-1.5">
            <span class="font-bold text-sky-300 truncate text-xs sm:text-sm">{{ towerStore.selectedPlacedTower.name
            }}</span>
          ...
          <div class="flex items-center gap-2.5 text-[10px] text-slate-400 font-mono mt-0.5">
            <span class="flex items-center gap-1">
              <Flame class="w-3 h-3 text-rose-400" /> {{ towerStore.selectedPlacedTower.damage }} DMG
            </span>
            <span class="flex items-center gap-1">
              <Crosshair class="w-3 h-3 text-sky-400" /> {{ towerStore.selectedPlacedTower.range }}k
            </span>
            <span class="flex items-center gap-1">
    ...
    <div class="flex items-center gap-2  pointer-events-auto" @mousedown.stop @mouseup.stop @click.stop @touchstart.stop
      ...
      <button v-for="bp in towerStore.blueprints" :key="bp.id" @click="selectTowerToBuild(bp)" :class="[
